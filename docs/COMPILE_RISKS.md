...
Цель: собрать `NordInvasion.dll` (Bannerlord 1.4.x) максимально с первого раза и
точечно закрыть CS-ошибки по `rgl_log.txt`. Этот файл — живой трекер: обновляется
по мере верификации по DLL.
